@props(['showRejectModal', 'showAcceptModal', 'method', 'request'])
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 mb-2">
                                    Silakan masukkan alasan penolakan untuk pengajuan ini.
                                </p>
                                <textarea wire:model="rejectionReason" rows="3"
                                    ...
                                    placeholder="Contoh: Nominal tidak sesuai dengan struk..."></textarea>
...
@if ($showAcceptModal)
    <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            {{-- Background Overlay --}}
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
                wire:click="$set('showAcceptModal', false)"></div>
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                {{-- Judul dinamis sesuai status Finance --}}
                                {{ $request->status->value === 'pending_finance' ? 'Konfirmasi Verifikasi COA' : 'Konfirmasi Persetujuan' }}
                            </h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 mb-2">
                                    {{-- Pesan dinamis sesuai status Finance --}}
                                    {{ $request->status->value === 'pending_finance'
                                        ? 'Apakah Anda yakin sudah memeriksa dan memastikan COA yang dipilih benar?'
                                        : 'Apakah Anda yakin ingin menyetujui pengajuan ini dan memprosesnya ke tahap selanjutnya?' }}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    {{-- Tombol ini akan mengeksekusi fungsi yang tersimpan di variabel $method --}}
                    <button type="button" wire:click="{{ $method }}" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="{{ $method }}">
                            {{ $request->status->value === 'pending_finance' ? 'Ya, Verifikasi' : 'Ya, Setujui' }}
                        </span>
                        <span wire:loading wire:target="{{ $method }}">Memproses...</span>
                    </button>
                    <button type="button" wire:click="$set('showAcceptModal', false)"
                        class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                        Batal
                    </button>
                </div>
